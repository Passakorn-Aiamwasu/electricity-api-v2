...
app.get('/api/usage/total-by-year', (req, res) => {
    ...
    const totals = data.reduce((acc, curr) => {
        ...
        const totalUsage = Object.keys(curr).filter(key => key.endsWith('_kwh')).reduce((sum, key) => sum + (curr[key] || 0), 0);
...
app.get('/api/users/total-by-year', (req, res) => {
    ...
    const totals = data.reduce((acc, curr) => {
        const year = curr.year;
        const totalUsers = Object.keys(curr).filter(key => key.endsWith('_count')).reduce((sum, key) => sum + (curr[key] || 0), 0);
